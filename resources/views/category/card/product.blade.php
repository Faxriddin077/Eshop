...
<ul class="product_list">
    <li class="products__item">
        ...
        <img src="{{ $product->main_image_url }}" alt="{{ $product->name }}" alt="phone" class="products__img">
        </a>
        <p class="products__text">{{ $product->name }}</p>
        <p class="products__price">{{ session()->get('default_currency')->symbol . number_format($product->price, 2) }}</p>
